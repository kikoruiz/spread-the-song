const config = {
  APPLE_MUSIC: {
    ...
    SONG_URL_PATTERN:
      /https:\/\/music.apple.com\/es\/album\/(?<albumName>.[^/]*)\/(?<albumId>\d+)\?i=(?<songId>\d+)/
  },
  MUSIXMATCH: {
    ...
    SONG_URL_PATTERN:
      /https:\/\/www.musixmatch.com\/lyrics\/(?<artistName>[A-Za-z0-9-]+)\/(?<songName>[A-Za-z0-9-]+)/
  },
  SPOTIFY: {
    ...
    SONG_URL_PATTERN:
      /https:\/\/open.spotify.com\/track\/(?<songId>[A-Za-z0-9]+)/
  },
  YOUTUBE: {
    ...
    SONG_URL_PATTERN:
      /https:\/\/www.youtube.com\/watch\?v=(?<songId>[A-Za-z0-9-_]+)/
  },
  YOUTUBE_MUSIC: {
    MEDIA_TYPE: 'audio',
    NAME: 'Youtube Music',
    ...
    SONG_URL_PATTERN:
      /https:\/\/music.youtube.com\/watch\?v=(?<songId>[A-Za-z0-9-_]+)&list=(?<listId>[A-Za-z0-9-_]+)/
  }
}
